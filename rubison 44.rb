...
OptionParser.new do |parser|
  ...
  parser.banner = "For Go Eat Only.rb [options]"
  ...
  parser.on("-m", "--map MAP", "Size of map") do |m|
    options[:map] = m.to_i
  end
  ...
  parser.on("-f", "--file Filename", "File Name") do |f|
    options[:input_file] = f
  end

end.parse!
$drivers_name = ["Arif","Agus","Sigit","Nadim","Budi"]
$stores_name = [["Mekdi", {"Burger" => 10,"Fries" => 20,"Cheese" => 30}], ["Xbucks", {"Frap" => 10, "Cap" => 10, "Americano" => 10}], ["TakuBel", {"Taco" => 9, "Quesadila" => 10, "Buritto" => 12}]]
...
class User
    ...
    def order_menu
        puts "Insert Menu Name|Quantity"
        input = gets.chomp
        while input != "ok"
            order = input.split("|")
